...
class TrackManager:
    ...
    def get_confirmed_tracks(self) -> Dict[int, KalmanFilterTrack]:
        ...
        return {
            ...
            if self.track_info[track_id].is_confirmed and not self.track_info[track_id].is_deleted
        }
    ...
    def get_track_states(self, confirmed_only: bool = False) -> List[Tuple[int, jnp.ndarray]]:
        """
    ...
    def _initialize_state_from_measurement(
        ...
        if measurement.shape[0] == 2:  # Position measurement
            ...
            initial_P = jnp.diag(jnp.array([R[0,0], R[1,1], 100.0, 100.0])) * covariance_scale
    ...
    def _remove_oldest_unconfirmed_track(self) -> None:
        ...
        unconfirmed_tracks = [
            (track_id, info) for track_id, info in self.track_info.items()
    ...
    def get_track_summary(self) -> Dict[str, int]:
        ...
        confirmed = sum(1 for info in self.track_info.values() if info.is_confirmed and not info.is_deleted)
        tentative = sum(1 for info in self.track_info.values() if not info.is_confirmed and not info.is_deleted)
