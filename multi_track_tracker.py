...
class TrackerConfig:
    ...
    data_association_algorithm: str = "nearest_neighbor"  # "nearest_neighbor" or "global_nearest_neighbor"
...
class MultiTrackTracker:
    ...
    def __init__(self, config: TrackerConfig):
        ...
        if config.data_association_algorithm == "nearest_neighbor":
            ...
            raise ValueError(f"Unknown data association algorithm: {config.data_association_algorithm}")
    ...
    def update(
        ...
        if dt is None:
            dt = self.current_time - self.last_update_time if self.last_update_time > 0 else 1.0
    ...
    def _associate_measurements(
        ...
        track_states_with_ids = self.track_manager.get_track_states(confirmed_only=False)
        ...
        track_idx_associations = self.data_associator.associate(track_states, measurements)
    ...
    def predict_tracks(self, dt: float) -> Dict[int, jnp.ndarray]:
        ...
        for track_id, track in self.track_manager.get_all_active_tracks().items():
            ...
            predicted_state = self.config.motion_model_class.transition(track.state.x, dt)
