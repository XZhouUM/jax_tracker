...
from data_association.multi_hypothesis_tracking import MultiHypothesisTracking, Hypothesis
...
def test_hypothesis_creation():
    ...
    hypothesis = Hypothesis(
        track_assignments={0: 0, 1: None},  # Track 0 -> Measurement 0, Track 1 -> No detection
...
def test_mht_basic():
    ...
    measurements = [
        (jnp.array([1.0, 2.0]), PositionMeasurement, jnp.eye(2) * 0.1)
    ]
...
def test_mht_multiple_tracks_measurements():
    ...
    measurements = [
        (jnp.array([0.1, 0.1]), PositionMeasurement, jnp.eye(2) * 0.1),  # Close to track 0
        (jnp.array([5.1, 5.1]), PositionMeasurement, jnp.eye(2) * 0.1),  # Close to track 1
        (jnp.array([10.0, 10.0]), PositionMeasurement, jnp.eye(2) * 0.1),  # Far from both
    ]
