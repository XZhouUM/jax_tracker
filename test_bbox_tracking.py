...
def test_bounding_box_measurement_model():
    ...
    state = jnp.array([100, 50, 40, 30, 2, 1, 0.5, 0.2])  # [cx, cy, w, h, vcx, vcy, vw, vh]
    measurement = BoundingBoxMeasurement.predict_measurement(state)
    
    expected_measurement = jnp.array([100, 50, 40, 30])  # [cx, cy, w, h]
    assert jnp.allclose(measurement, expected_measurement), f"Expected {expected_measurement}, got {measurement}"
...
def test_bounding_box_motion_model():
    ...
    assert jnp.allclose(next_state, expected_next_state), f"Expected {expected_next_state}, got {next_state}"
    ...
    assert jnp.allclose(initial_state, expected_initial_state), f"Expected {expected_initial_state}, got {initial_state}"
...
def test_iou_association():
    ...
    measurements = [
        (jnp.array([105, 52, 42, 32]), BoundingBoxMeasurement, jnp.eye(4)),  # Close to track 1
        (jnp.array([195, 105, 48, 38]), BoundingBoxMeasurement, jnp.eye(4)),  # Close to track 2
        (jnp.array([300, 300, 20, 20]), BoundingBoxMeasurement, jnp.eye(4)),  # Far from both
    ]
    ...
    assert associations_nn[0] == 0, f"Track 0 should associate with measurement 0, got {associations_nn[0]}"
    assert associations_nn[1] == 1, f"Track 1 should associate with measurement 1, got {associations_nn[1]}"
    ...
    assert associations_opt == associations_nn, f"Optimal assignment should match NN for this case"
...
def test_bbox_tracker_integration():
    ...
    for step, measurements in enumerate(measurements_sequence):
        ...
        if step == 0:
            ...
            assert result['track_summary']['tentative'] >= 1, f"Step {step}: Expected at least 1 tentative track"
        elif step == 1:
            # Should still have 1 track (confirmed or tentative)
            assert result['track_summary']['total'] >= 1, f"Step {step}: Expected at least 1 track"
        elif step == 2:
            # Should have 2 tracks now
            assert result['track_summary']['total'] >= 2, f"Step {step}: Expected at least 2 tracks"
        elif step == 3:
    ...
    assert tracks_created >= 2, f"Should have created at least 2 tracks, got {tracks_created}"
...
def test_iou_threshold_behavior():
    ...
    measurements = [
        (jnp.array([110, 55, 35, 25]), BoundingBoxMeasurement, jnp.eye(4)),  # Moderate overlap
    ]
...
def test_edge_cases():
    ...
    assert len(associations) == 0, "Should return empty associations for no measurements"
    ...
    assert len(associations) <= 1, "Should only associate with bounding box measurements"
