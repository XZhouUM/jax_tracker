...
class MeasurementPreprocessor:
    ...
    def _validate_measurement(self, measurement: jnp.ndarray, sensor_info: SensorInfo) -> bool:
        """
    ...
    def _transform_measurement(self, measurement: jnp.ndarray, sensor_info: SensorInfo) -> jnp.ndarray:
        """
    ...
    def simulate_measurements(
        ...
        for sensor_id, sensor_info in self.sensors.items():
            sensor_measurements = self._simulate_sensor_measurements(
                sensor_id, sensor_info, true_states, timestamp,
                detection_probability, false_alarm_rate, key
            )
    ...
    def _simulate_sensor_measurements(
        ...
        for i, state in enumerate(true_states):
            ...
            if jax.random.uniform(subkey) < detection_probability:
                ...
                true_measurement = sensor_info.measurement_model.predict_measurement(state)
