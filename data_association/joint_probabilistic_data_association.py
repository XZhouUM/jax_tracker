...
class JointProbabilisticDataAssociation(DataAssociation):
    ...
    def _calculate_association_probabilities(
        ...
        prob_matrix, prob_undetected, prob_false_alarm = self._calculate_marginal_probabilities(
    ...
    def _build_validation_matrix(
        ...
        for track_idx, track_state in enumerate(track_states):
            for meas_idx, (measurement, measurement_model, _) in enumerate(measurements):
                ...
                validation_matrix = validation_matrix.at[track_idx, meas_idx].set(is_valid)
        ...
        for track_idx, track_state in enumerate(track_states):
            for meas_idx, (measurement, measurement_model, R) in enumerate(measurements):
                if validation_matrix[track_idx, meas_idx]:
                    ...
                    likelihood_matrix = likelihood_matrix.at[track_idx, meas_idx].set(likelihood)
    ...
    def _calculate_combined_updates(
        ...
        for track_idx in range(n_tracks):
            ...
            weighted_innovation = jnp.zeros_like(measurements[0][0])  # Initialize with measurement dimension
            total_prob = 0.0
            ...
            for meas_idx, (measurement, measurement_model, R) in enumerate(measurements):
                if validation_matrix[track_idx, meas_idx]:
                    ...
                    if prob > 1e-10:  # Only consider significant probabilities
                        predicted_measurement = measurement_model.predict_measurement(track_state)
                        innovation = measurement - predicted_measurement

                        weighted_innovation += prob * innovation
                        total_prob += prob

                        valid_measurements.append((measurement, measurement_model, R, innovation))
        ...
        for track_idx in range(n_tracks):
            ...
            for meas_idx in range(n_measurements):
                if meas_idx not in used_measurements:
                    prob = prob_matrix[track_idx, meas_idx]
                    if prob > best_prob and prob > 0.5:  # Threshold for confident association
                        best_prob = prob
